...
namespace PWMIS.Web.Controls
{
    ...
    public class DataTextBox : TextBox, IDataControl, IQueryControl
	{
...
//				}
		...
		protected override void OnPreRender(EventArgs e)
		{
            ...
            else
            {
                if (!this.isNull )
                {    //不可为空
                    this.Page.ClientScript.RegisterOnSubmitStatement(this.GetType (),this.UniqueID, "if(document.all." + this.UniqueID + ".value==''){ShowMessage('该项不能为空!',document.all." + this.UniqueID + ",'" + messageType + "');document.all." + this.UniqueID + ".focus();return false;}");
                }
